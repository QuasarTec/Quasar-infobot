...
module.exports = async (req,res) => {
    ...
    //Делаем сообщение для зашифровки
    let message = username + ' https://www.youtube.com/watch?v=dQw4w9WgXcQ ' + Math.floor(Math.random()*10000000);

    //Хэшируем
    let hash = md5(message);

    const insert_hash = `INSERT INTO redirect (user_id, hash) VALUES (${user_id.rows[0].id}, '${hash}')`;
