...
router.use(async (req, res, next) => {
  if (
    SECRET_TOKEN !== req.body.token &&
    req.method === "POST" &&
    req.url.includes("admin-panel")
  ) {
...
router.get("/admin-panel/users/newsletters", async (req, res) => {
  fs.readFile(
    __dirname + "/static/assets/newsletters.html",
    async (err, data) => {
      if (err) throw err;
...
router.post("/admin-panel/users/activate", (req, res) => {
  require("../../controllers/admin-panel/activete_and_deactivete")(
    req,
    res,
    "Now()"
  );
