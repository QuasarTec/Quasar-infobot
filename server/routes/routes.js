...
router.get('/referrals-vizualization', async (req, res) => {
    ...
    if (response.rowCount === 0) {
        res.send(`Ошибка! Мы не можем найти пользователя с ником @${root}\n<a href="https://meet.qtconnect.ru" target="_blank">Главная</a>`);
        return
    }
...
router.post('/pay/confirm', (req, res) => {
    ...
    client.query(query, async (err, response) => {
        ...
        client.query(query, (err) => {
            ...
            bot.sendMessage(response.rows[0].chat_id, "Оплата прошла успешно!\nВы можете пользоваться технологией Connect")
            ...
            client.query(query, async (err, res) => {
                ...
                if (res.rowCount === 0) {
                    ...
                    client.query(query);
                    test_period_sevices = ['qcloud_pay', 'franchise_pay', 'message_pay', 'insta_comment_pay', 'insta_lead_pay', 'skype_lead_pay', 'skype_reg_pay', 'tele_lead_pay', 'vk_lead_pay', 'vk_reg_pay', 'insta_king_pay'];
                } else {
                    let sevices = Object.keys(res.rows[0]);

                    sevices.forEach(service => {
                        if (service !== 'id' && service !== 'user_id' && (res.rows[0][service] === null || parseInt((new Date()-res.rows[0][service])/(24*3600*1000)) > 30)) {
                ...
                test_period_sevices.forEach(service => {
                    if (service === 'last_pay') {
                        let query = `UPDATE quasar_telegrambot_users_new SET last_pay = '${new Date(new Date().getTime() - (2*7*24*3600*1000)).toUTCString()}', `;
            ...
            bot.sendMessage(response.rows[0].chat_id, 'В честь запуска бота, мы дарим вам пробный период, на все сервисы компании на срок в 2 недели!');
...
router.get('/users/get-balance/', (req,res) => {
	...
	query = `SELECT last_pay FROM quasar_telegrambot_users_new WHERE username = '${username}'`
        ...
        client.query(query, (err, response) => {
			...
			let balance = 12 - (12/30) * parseInt((new Date()-response.rows[0].last_pay)/(24*3600*1000));
                        res.json({
