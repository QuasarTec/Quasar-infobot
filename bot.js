...
bot.on('callback_query', async (callbackQuery) => {
    ...
    if (action === 'traffic') {
        text =
            'Всего 2 шага: \n 1. Необходимо авторизоваться или зарегистрироваться по ссылке https://quasaria.ru/ \n 2. Переходи по ссылке: https://quasaria.ru/shop/star/EasyTrafficSoft  \n 3. Покупай звезду за 0₽ и получай первую 1000 лицензий! \n После выполнения пунктов пропиши /check \n Человечеству от Quasar Technology';
    } else if (action === 'home') {
        text = 'Главная';
        ...
        const response = referrals.transformRefs(
            await referrals.getAllReferals([res.rows[0].id], 9)
        );
        ...
        if (action.split('_').length > 1) {
            ...
            opts.reply_markup = JSON.stringify({
                inline_keyboard: [
                    ...
                    [
                        { text: 'Общий список', callback_data: `refs_list_${service}` },
                        { text: 'Список активных', callback_data: `active_refs_list_${service}` },
                    ],
                    [
                        {
                            text: 'Назад',
            ...
            opts.reply_markup = JSON.stringify({
                inline_keyboard: [
                    [{ text: 'Визуальный просмотр', url: link }],
                    [
                        { text: 'Общий список', callback_data: 'refs_list' },
                        { text: 'Список активных', callback_data: `active_refs_list` },
                    ],
                    [{ text: 'Назад', callback_data: 'account' }],
                    [{ text: 'Главное меню', callback_data: 'main' }],
                ],
        ...
        let new_msg_id = (await bot.sendMessage(msg.chat.id, 'Подождите, файл загружается'))
            .message_id;
        await bot.sendDocument(
            msg.chat.id,
            `${__dirname}/commands/static/exe/Quasar\ Message\ Setup\ 1.8.0.exe`,
            {
                caption: `Программы для автоматизации бизнеса так же запускаются из мессенджера. Подробно ознакомиться с их функционалом Вы сможете тапнув по кнопке "Автоматизация", или на сайте quasaria.ru в разделе "Магазин звезд"\n\nДля дальнейшего пользования продуктами, Вам необходимо: скачать, распаковать и установить архив Quasar Message `,
            }
        );
        ...
        await bot.sendDocument(
            msg.chat.id,
            `${__dirname}/commands/static/rar/services/VKConnect.rar`,
            {
                parse_mode: 'HTML',
        ...
        fs.readFile(`${__dirname}/commands/static/html/qcloud.html`, async (err, data) => {
            ...
            fs.readFile(
                `${__dirname}/commands/static/html/QCloud_v1.1.0.html`,
                async (err, data) => {
        ...
        bot.sendMessage(
            msg.chat.id,
            'Если у вас возникли проблемы в работе с ботом, вы можете прописать /start и попробовать повторить сои действия, если это не помогло, вы можете обратиться за помощью на сайте https://quasaria.ru/contacts/'
        );
