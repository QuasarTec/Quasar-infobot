...
bot.on("callback_query", async (callbackQuery) => {
  ...
  if (action === "traffic") {
    ...
    await bot.sendPhoto(
      msg.chat.id,
      `${__dirname}/commands/static/img/main.jpg`,
      opts
    );
  } else if (action === "pay") {
    ...
    let messageText = fs.readFileSync(
      `${__dirname}/commands/static/html/func.html`
    );
    bot.sendMessage(msg.chat.id, messageText, options);
  } else if (action === "message_download") {
    let new_msg_id = (
      await bot.sendMessage(msg.chat.id, "Подождите, файл загружается")
    ).message_id;
    ...
    await bot.sendPhoto(
      msg.chat.id,
      `${__dirname}/commands/static/img/marketing_connect.jpg`,
      {
        parse_mode: "HTML",
    ...
    fs.readFile(
      `${__dirname}/commands/static/html/connect_marketing.html`,
      (err, data) => {
        if (err) throw err;
    ...
    await bot.sendPhoto(
      msg.chat.id,
      `${__dirname}/commands/static/img/connect_funcs.jpg`
    );
    fs.readFile(
      `${__dirname}/commands/static/html/connect_desk.html`,
      (err, data) => {
        if (err) throw err;
        bot.sendMessage(msg.chat.id, data, {
          parse_mode: "HTML",
    ...
    await bot.sendPhoto(
      msg.chat.id,
      `${__dirname}/commands/static/img/gift_license.jpg`,
      {
    ...
    fs.readFile(
      `${__dirname}/commands/static/html/qcloud.html`,
      async (err, data) => {
        let opts = {};
        ...
        await bot.sendPhoto(
          msg.chat.id,
          `${__dirname}/commands/static/img/qcloud.jpg`,
          {
            parse_mode: "HTML",
            caption: data,
          }
    ...
    await bot.sendPhoto(
      msg.chat.id,
      `${__dirname}/commands/static/img/first_steps.jpg`,
      opts
    );
    ...
    await bot.sendPhoto(
      msg.chat.id,
      `${__dirname}/commands/static/img/about_company.jpg`
    );
    ...
    fs.readFile(
      `${__dirname}/commands/static/html/navigate.html`,
      (err, data) => {
        if (err) throw err;
        bot.sendMessage(msg.chat.id, data, {
          parse_mode: "HTML",
...
bot.on("new_chat_members", async (msg) => {
  ...
  if (
    msg.new_chat_participant.username === "quasar_infobot" ||
    res.rowCount === 0
  ) {
      ...
      if (res.rowCount === 0) {
        ...
        for (let i = 0; i < res.rowCount; i++) {
          ...
          text += `ID: ${res.rows[i].id}\nИмя пользователя: ${
            ...
            res.rows[i].is_admin
          }\nСигнатура для оплаты: ${
            res.rows[i].sign
          }\nПргласитель: ${inviter}\n\nОплачено: ${
            parseInt(
              (new Date() - res.rows[i].last_pay) / (24 * 3600 * 1000)
            ) <= 30
              ? "Оплачено"
              : "Не оплачено"
          }`;
...
const sendConfirmationMessage = (message, isNews = false) => {
  ...
  bot.sendMessage(
    message.chat.id,
    isNews ? "Сделать новость?" : "Сделать рассылку?",
    send_opts
  );
};
