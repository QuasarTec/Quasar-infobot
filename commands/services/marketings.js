...
module.exports = async (data, service) => {
    let pathToCaption = path.join(__dirname, '..', 'static/html/marketings/photo_caption', `${service}.html`);

    let pathToPhoto = path.join(__dirname, '..', 'static/img/marketings/', `${service}_marketing.jpg`);
    await fs.readFile(pathToCaption, async (err, text) => {
        ...
        fs.readFile(pathToExemple, (err, text) => {
            ...
            data.bot.sendMessage(data.msg.chat.id, text, {
                ...
                reply_markup: {
                    inline_keyboard: [
                        [{text: 'Назад', callback_data: `service_${service}`}]
                    ]
                }
            })
        })
