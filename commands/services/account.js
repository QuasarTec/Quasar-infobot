...
module.exports = async (data, service_callback, service, db_field_name) => {
    ...
    if (res.rowCount === 0) {
        ...
        response.opts.reply_markup = JSON.stringify({
            inline_keyboard: [
                [{ text: 'Количество рефералов', callback_data: `refs_count_${service}` }, { text: 'Просмотреть рефералов', callback_data: `refs_${service}`}],
                [{ text: 'Ваш пригласитель', callback_data: `inviter_${service}` }, { text: 'Ваш наставник', callback_data: `mentor_${service}`}],
