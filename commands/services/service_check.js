...
module.exports = async (
  ...
  if (res.rowCount === 0) {
    sending_msg.opts.reply_markup = {
      inline_keyboard: [
        [
          {
            text: "Маркетинг",
            callback_data: `marketing_${callback_name.replace("pay_", "")}`,
          },
        ],
    ...
    res.rowCount !== 0 &&
    parseInt((new Date() - res.rows[0][db_field_name]) / (24 * 3600 * 1000)) >
      30
  ) {
    sending_msg.text +=
    ...
    sending_msg.opts.reply_markup = {
      inline_keyboard: [
        ...
        [
          {
            text: "Личный кабинет",
            callback_data: `account_${callback_name.replace("pay_", "")}`,
          },
        ],
        [{ text: "Назад", callback_data: "services" }],
        [{ text: "Главная", callback_data: "main" }],
  ...
  let pathToPhotoCaption = path.join(
    __dirname,
    "..",
    "/static/html/services/photo_caption",
    db_field_name.replace("_pay", ".html")
  );

  let pathToFileCaption = path.join(
    __dirname,
    "..",
    "/static/html/services/file_caption",
    db_field_name.replace("_pay", ".html")
  );
  ...
  fs.readFile(pathToFileCaption, async (err, text) => {
    ...
    if (err) {
      fs.readFile(pathToPhotoCaption, async (err, text) => {
        ...
        let pathToPhoto = __dirname.replace(
          "services",
          `static/img/services/${db_field_name.replace("_pay", ".jpg")}`
        );
    ...
    let pathToFile = __dirname.replace(
      "services",
      `static/rar/services/${name.replace(" ", "")}.rar`
    );
