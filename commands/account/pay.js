...
module.exports = async (msg,bot,pay_name='last_pay',amount=960,curr='RUB') => {
    ...
    if (res.rowCount === 0) {
        ...
        let response = await axios({
            method: "post",
            url: "https://api.easy-stars.ru/api/pay/get_pay_link",
            headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
            data: `token=${encodeURIComponent(token)}&username=${encodeURIComponent('@'+msg.chat.username)}&m_curr=${curr}&m_amount=${amount}&desc=${pay_name}`
        ...
        if (response.data.status === "error") {
            ...
            client.query(query, (err, res) => {
                if (err) {
                    bot.sendMessage(msg.chat.id, 'Похоже что-то сломалось, обратитесь за помощью на сайте', opts)
                    return;
                }
            })
