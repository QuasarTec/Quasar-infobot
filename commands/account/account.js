...
module.exports = async (msg, opts) => {
  ...
  if (res.rowCount === 0) {
    ...
    response.text =
      "Похоже вы ещё не оплатили использование реферальной системы";
    ...
    parseInt((new Date() - res.rows[0].last_pay) / (24 * 3600 * 1000)) > 30
  ) {
    ...
    response.opts.reply_markup = JSON.stringify({
      inline_keyboard: [
        [
          { text: "Количество рефералов", callback_data: "refs_count" },
          { text: "Просмотреть рефералов", callback_data: "refs" },
        ],
        [
